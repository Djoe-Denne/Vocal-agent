...
use asr_domain::{
    AlignmentOutput, AlignmentPort, AlignmentRequest, AudioChunk, DomainError, DomainEvent,
    PipelineContext, PipelineStage, WordTiming,
};
...
impl Wav2Vec2ModelConfig {
    fn load(path: &Path) -> Result<Self, DomainError> {
        let data = std::fs::read_to_string(path)
            .map_err(|e| DomainError::internal_error(&format!("read config.json: {e}")))?;
        serde_json::from_str(&data)
            .map_err(|e| DomainError::internal_error(&format!("parse config.json: {e}")))
    }
...
impl FeatureProjection {
    fn load(cfg: &Wav2Vec2ModelConfig, vb: VarBuilder) -> candle_core::Result<Self> {
        let dim = *cfg.conv_dim.last().unwrap();
        Ok(Self {
...
impl Wav2Vec2ForcedAligner {
    pub fn load(adapter_cfg: &Wav2Vec2AdapterConfig) -> Result<Self, DomainError> {
        let device = match adapter_cfg.device.as_str() {
            "cuda" => Device::new_cuda(0).map_err(|e| {
                DomainError::internal_error(&format!("CUDA init failed: {e}"))
            })?,
            _ => Device::Cpu,
        };

        let model_cfg = Wav2Vec2ModelConfig::load(Path::new(&adapter_cfg.config_path))?;
        let frame_stride_ms = model_cfg.frame_stride_ms(16_000);
        let blank_id = model_cfg.pad_token_id;

        let vocab = load_vocab(Path::new(&adapter_cfg.vocab_path))?;
        let word_sep_id = vocab.get(&'|').copied().unwrap_or(0);

        let model_data = std::fs::read(&adapter_cfg.model_path).map_err(|e| {
            DomainError::internal_error(&format!("read safetensors: {e}"))
        })?;
        let vb =
            VarBuilder::from_buffered_safetensors(model_data, DType::F32, &device).map_err(
                |e| DomainError::internal_error(&format!("load safetensors: {e}")),
            )?;

        let model = Wav2Vec2ForCTC::load(&model_cfg, vb)
            .map_err(|e| DomainError::internal_error(&format!("build model: {e}")))?;
    ...
    fn do_align(
        ...
        audio: &AudioChunk,
        transcript: &str,
    ) -> Result<Vec<WordTiming>, DomainError> {
        if audio.samples.is_empty() || transcript.trim().is_empty() {
            return Ok(Vec::new());
        }
        ...
        if audio.sample_rate_hz != 16_000 {
            tracing::warn!(
                ...
                "wav2vec2 expects 16 kHz; alignment quality may degrade"
            );
        }

        let normalized = normalize_audio(&audio.samples);

        let audio_tensor =
            Tensor::from_vec(normalized, (1, audio.samples.len()), &self.device).map_err(|e| {
                DomainError::internal_error(&format!("tensor creation: {e}"))
            })?;

        let logits = self
            .model
            .forward(&audio_tensor)
            .map_err(|e| DomainError::internal_error(&format!("forward pass: {e}")))?;

        let log_probs_t = candle_nn::ops::log_softmax(&logits, D::Minus1)
            .and_then(|t| t.squeeze(0))
            .map_err(|e| DomainError::internal_error(&format!("log_softmax: {e}")))?;

        let log_probs: Vec<Vec<f32>> = log_probs_t
            .to_vec2()
            .map_err(|e| DomainError::internal_error(&format!("to_vec2: {e}")))?;
...
fn load_vocab(path: &Path) -> Result<HashMap<char, usize>, DomainError> {
    let data = std::fs::read_to_string(path)
        .map_err(|e| DomainError::internal_error(&format!("read vocab.json: {e}")))?;
    let raw: HashMap<String, usize> = serde_json::from_str(&data)
        .map_err(|e| DomainError::internal_error(&format!("parse vocab.json: {e}")))?;
...
impl AlignmentPort for Wav2Vec2ForcedAligner {
    async fn align(&self, request: AlignmentRequest) -> Result<AlignmentOutput, DomainError> {
        ...
        let words = self.do_align(&request.audio, &transcript_text)?;
...
pub struct Wav2Vec2AlignmentStage {
    aligner: Wav2Vec2ForcedAligner,
}

impl Wav2Vec2AlignmentStage {
    pub fn new(aligner: Wav2Vec2ForcedAligner) -> Self {
        Self { aligner }
    }
}

#[async_trait]
impl PipelineStage for Wav2Vec2AlignmentStage {
