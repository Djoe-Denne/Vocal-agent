# PowerShell script to set up the OpenClaw Podman container (Windows)
...
$SHARED_DIR = Join-Path $PROJECT_DIR "plugins"

if (-not (Test-Path $SHARED_DIR)) {
    New-Item -ItemType Directory -Force -Path $SHARED_DIR | Out-Null
    Write-Host "[container] Created plugins folder: $SHARED_DIR" -ForegroundColor Yellow
}
...
if ($existingContainer -eq $CONTAINER_NAME) {
    ...
    podman build -t $IMAGE_NAME -f (Join-Path $PROJECT_DIR "Containerfile") $PROJECT_DIR
    ...
    Write-Host "[container] Mounting shared folder: $SHARED_DIR -> /app/plugins" -ForegroundColor Yellow
    podman run -d --name $CONTAINER_NAME --network host -p 1455:1455 -p 18789:18789 -v "${SHARED_DIR}:/app/plugins:z" $IMAGE_NAME
