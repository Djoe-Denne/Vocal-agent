...
$serviceNames = @("audio-service", "asr-service", "alignment-service", "tempo-service", "orchestration-service")
...
Start-Process cargo -WorkingDirectory (Join-Path $repoRoot "tempo-service") -ArgumentList "run -p tempo-setup --bin tempo-service"
