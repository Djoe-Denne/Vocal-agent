...
Configuration dataclasses and loader for the TTS server.
...
class ModelConfig:
    ...
    voices_dir: Path = Path("./tts_python/voices")
...
class TTSConfig:
    ...
    # Pipeline definitions (optional, loaded from [pipelines.*] / [stages.*])
    pipelines_raw: dict = field(default_factory=dict)
    stages_raw: dict = field(default_factory=dict)
...
def load_config(config_path: Optional[Path] = None) -> TTSConfig:
    ...
    if config_path is None:
        # Walk up from application/ -> tts_python/ -> project root
        config_path = Path(__file__).resolve().parent.parent.parent / "tts.toml"
